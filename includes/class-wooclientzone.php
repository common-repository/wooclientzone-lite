...
class Wooclientzone {
	...
	private function define_admin_hooks() {
		...
		$this->loader->add_filter( 'plugin_action_links_' . "wooclientzone-lite/wooclientzone.php", $plugin_admin, 'plugin_action_links' );
		...
		// For footer rating link
		$this->loader->add_action('wp_ajax_admin_rating_link_clicked', $plugin_admin, 'admin_rating_link_clicked');
		// Filter used to inject an empty string to the footer text of wooclientzone pages (settings and clientzones) to avoid flashing the pre-existing
		// text (from woocommerce or wordpress) before we add our with javascript (we use javascript to add Ajax functionality to footer links
	...
	private function define_public_hooks() {
		...
		// this is only going to be hooked if both userzones and orderzones are enabled (order eligibility is checked within the callback function)
		...
		$this->loader->add_action('init', $plugin_public, 'my_account_clientzone_add_rewrite_endpoint');
		$this->loader->add_filter('query_vars', $plugin_public, 'my_account_clientzone_add_query_var', 0);
		if (get_option('wooclientzone_use_userzones') == 'yes') {
		...
		$this->loader->add_action('woocommerce_account_clientzone_endpoint', $plugin_public, 'my_account_clientzone_endpoint_content');
